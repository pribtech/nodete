...
QUERY_BUILDER.set('ODBC_SolidDB', {
	buildSelect: function(queryBase, fetchFirst) {
		var columns = this.buildColumnList(queryBase);
		if(columns == "" || columns == null) return null;
		
		// handling of solid admin commands for list_Table. a bit of a kludge.
		if (queryBase.queryType != null && queryBase.queryType.toLowerCase() == 'inlinequery') {
			if (queryBase.getBaseQuery().substring(0,5) == 'admin') {
	...
	buildWhereClause: function(queryBase) {
		...
		if(rootNode != null)
		{
				...
				for(i = 0; i < rootNode.groupNodes.length || nodeStack.length != 0; i++)
				{
					...
					else
					{
						if(rootNode.groupNodes[i].column != "" && rootNode.groupNodes[i].compareType != "" && (rootNode.groupNodes[i].value != "" || Object.isNumber(rootNode.groupNodes[i].value)) )
						{
							...
							whereClause += this.buildCompareType(queryBase.components.column[rootNode.groupNodes[i].column].sql_name,rootNode.groupNodes[i].compareType,rootNode.groupNodes[i].value);
							writenItems++;
						}
					}
			...
			else
			{
				...
				{
					if(writenItems != 0) whereClause += " " + Operand;
					whereClause += this.buildCompareType(queryBase.components.column[rootNode.column].sql_name,rootNode.compareType,rootNode.value);
					writenItems++;
				}
			}
		}
		...
		for(i=0; i<keyLength; i++)
		{
			...
			if(tableColumnRenderingModule != null)
			{
				if(tableColumnRenderingModule.containsWhereClauseForQuery == true)
				{
					returnedWhereList = "";
					returnedWhereList = tableColumnRenderingModule.whereClauseQueryPortion(queryBase);
					returnedWhereList = returnedWhereList.replace(/(^\s*)|(\s*$)/g, '');
					if(returnedWhereList.length > 0 && whereClause.length > 0)
					{
						whereClause = "(" + whereClause + ") AND (" + returnedWhereList + ")";
					}
					else if(returnedColumnList.length > 0 && whereClause.length == 0)
					{
	...
	buildCompareType: function(column,op,value) {
		switch (op.toLowerCase()) {
			case 'locate' :			return " LOCATE("+value+","+ column+") > 0";
			case 'locateanycase' :	return " (LOCATE(upper("+value+"),"+ column+",1, CODEUNITS16) > 0"
											+ " or LOCATE(lower("+value+"),"+ column+",1, CODEUNITS16) > 0)";
			default: 				return " " + column + " " + op + value;
		}
	}
});
