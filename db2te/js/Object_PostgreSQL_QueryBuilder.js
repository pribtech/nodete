...
QUERY_BUILDER.set('PostgreSQL', {
	buildSelect: function(queryBase, fetchFirst) {
		switch (queryBase.queryType.toLowerCase()) {
			case 'inlinequery':
				var query=queryBase.getBaseQuery();
				switch (getWord(query,1).toLowerCase()) {
					case 'select':
					case 'with':
					case 'value':
					case 'values':
					case 'call':
						break;
					default:
						return this.insertParameters(encodeMessage(query, $H(queryBase.tableCallParameters)), queryBase);
				}
		}
