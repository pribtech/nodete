...
class Statement_MQ extends Statement {

	function __construct($stmt_text, $prepare_statment = FALSE, $verbose = FALSE, $ForwardOnlyScroll = FALSE, $getRowCount = FALSE, $dbconn = null) {
		...
		$this->rows = -1;
		$splitStmt = explode(" ", $stmt_text, 2);
		$this->stmt=$splitStmt[0];
		...
		try {
			if ($prepare_statment) { 
				...
				$this->type = "Execute";
				java_throw_exceptions(true);
				$this->resultSetStmt = $this->dbconn->executeQuery($this->stmt);
	...
	function execute($parameters=null, $verbose=false) { // Execute, to only be used if previously prepared
		...
		try {
			java_last_exception_clear();
			$this->resultSetStmt = $this->dbconn->execute();
	...
	public function executeStmtWithParameters($bindParameters) {
				...
				if($isNull)
					...
					switch(strtolower($type)) {
						case "null":
							...
							break;
						case "constant":
							$this->dbconn->setParameterConstant($name , $value);
							break;
						case "list":
							$this->dbconn->setParameterList($name , $value);
							break;
						default:
							$this->dbconn->setParameter($name , $value);
					}
				if(java_last_exception_get()!=null) 
					throw new Exception('MQ Set parameter failed for '.$name.' error: '.java_last_exception_get());
			} catch (JavaException $e) {
				$this->checkForError($e);
				$this->sqlstate = '99999';
				return;
		...
		$startTime = microtime(true);	
		try {	
			$this->result = $this->dbconn->execute();
		} catch (JavaException $e) {
			$this->checkForError($e);
		} 
