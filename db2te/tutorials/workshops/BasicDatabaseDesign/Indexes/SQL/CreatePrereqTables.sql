...
CREATE TABLE ?SCHEMA?.customer_c (
  customer_ID SMALLINT NOT NULL,
  customer_name VARCHAR(50),
  customer_address VARCHAR(50),
  PRIMARY KEY (customer_ID));

INSERT INTO ?SCHEMA?.customer_c VALUES ( 2010, 'John', '24 Ontario' ),
...
CREATE TABLE ?SCHEMA?.order_master_c (
  ...
  CONSTRAINT fk_custid FOREIGN KEY (customer_ID)
    REFERENCES customer_c (customer_ID) ON DELETE CASCADE,
  CONSTRAINT fk_orderstatus FOREIGN KEY (order_status)
    REFERENCES order_status_c (order_status) ON DELETE CASCADE );

INSERT INTO ?SCHEMA?.order_master_c VALUES ( 11010, 2010, '2009-01-12', 6000, 'Cash', 1),
...
CREATE TABLE ?SCHEMA?.order_details_c (
  order_details_ID SMALLINT NOT NULL,
  product_ID SMALLINT NOT NULL,
  product_quantity INTEGER,
  total_cost DECIMAL(7,2),
  CONSTRAINT fk_orderid FOREIGN KEY (order_details_ID)
    REFERENCES order_master_c (order_master_ID) ON DELETE CASCADE,
  CONSTRAINT fk_prod FOREIGN KEY (product_ID)
    REFERENCES product_c (product_ID)ON DELETE CASCADE);

INSERT INTO ?SCHEMA?.order_details_c VALUES ( 11010, 1, 1, 6000),
						       ( 11011, 3, 1, 35000),
						       ( 11012, 2, 1, 25000),
						       ( 11013, 5, 1, 5000),
						       ( 11014, 2, 1, 25000),
