...
call admin_cmd('REORG TABLE "<xsl:value-of select="key[@name ='TABSCHEMA']/text()"/>"."<xsl:value-of select="key[@name ='TABNAME']/text()"/>" ALLOW NO ACCESS')<xsl:value-of select="$delimiter"/>
call admin_cmd('runstats on table "<xsl:value-of select="key[@name ='TABSCHEMA']/text()"/>"."<xsl:value-of select="key[@name ='TABNAME']/text()"/>"
			<xsl:choose>
				<xsl:when test="data[@name ='STATISTICSPROFILE']/text()!='' or data[@name ='STATISTICSPROFILE']/text()!=''">use profile</xsl:when>
