...
<xsl:param name="includeCreateTable">true</xsl:param>
<xsl:param name="includeIndices">true</xsl:param>
<xsl:param name="includePrimaryKey">true</xsl:param>
<xsl:param name="includeForeignKeys">true</xsl:param>
<xsl:param name="includeGrants">true</xsl:param>
<xsl:param name="tableNamePrefix"/>
<xsl:param name="tableNameSuffix"/>
<xsl:param name="indexNamePrefix"><xsl:value-of select="$tableNamePrefix"/></xsl:param>
<xsl:param name="indexNameSuffix"><xsl:value-of select="$tableNameSuffix"/></xsl:param>
<xsl:param name="schema"><xsl:value-of select="/table/@TABSCHEMA"/></xsl:param>
<xsl:param name="tableName"><xsl:value-of select="/table/@TABNAME"/></xsl:param>
<xsl:param name="tablespace"><xsl:value-of select="/table/@TBSPACE"/></xsl:param>
<xsl:param name="indexspace"><xsl:value-of select="/table/@INDEX_TBSPACE"/></xsl:param>
<xsl:param name="longspace"><xsl:value-of select="/table/@LONG_TBSPACE"/></xsl:param>

<xsl:variable name="table">"<xsl:value-of select="$schema"/>"."<xsl:value-of select="$tableNamePrefix"/><xsl:value-of select="$tableName"/><xsl:value-of select="$tableNameSuffix"/>"</xsl:variable>


<xsl:template match="/">
<xsl:value-of select="$commentTitle"/>  
begin<xsl:value-of select="$delimiter"/><xsl:value-of select="$newLine"/>
	<xsl:apply-templates select="table"/>
end<xsl:value-of select="$delimiter"/><xsl:value-of select="$newLine"/>
</xsl:template>

<xsl:template match="table">

<xsl:if test="$includeCreateTable='true'">
CREATE TABLE <xsl:value-of select="$table"/>
	(<xsl:apply-templates select="columns"/>	)
	<xsl:choose>
		<xsl:when test="@TBSPACE != ''">IN "<xsl:value-of select="$tablespace"/>"<xsl:if test="$indexspace != ''"> INDEX IN "<xsl:value-of select="$indexspace"/>"</xsl:if><xsl:if test="$longspace != ''"> LONG IN "<xsl:value-of select="$longspace"/>"</xsl:if>
		</xsl:when>
		<xsl:otherwise>IN ????????????????? to be done</xsl:otherwise>	
	</xsl:choose>
...
<xsl:if test="@VOLATILE='Y'"><xsl:value-of select="$newLine"/>ALTER TABLE  <xsl:value-of select="$table"/> VOLATILE<xsl:value-of select="$delimiter"/></xsl:if>
</xsl:if>
<xsl:if test="$includeIndices='true'">
-- indices
	<xsl:apply-templates select="indices"/>
</xsl:if>
<xsl:if test="$includePrimaryKey='true'">
-- primary key
	<xsl:apply-templates select="primaryKey"/>
</xsl:if>
<xsl:if test="$includeForeignKeys='true'">
-- foreign keys
	<xsl:apply-templates select="foreignKeys"/>
</xsl:if>
<xsl:if test="$includeGrants='true'">
-- grants
...
<xsl:template match="column">
	...
	</xsl:choose>"<xsl:value-of select="@COLNAME"/>" <xsl:value-of select="@TYPENAME"/> 
	<xsl:choose>
  		<xsl:when test="@TYPENAME='VARCHAR' or @TYPENAME='CHARACTER' or @TYPENAME='CLOB' or @TYPENAME='BLOB'">(<xsl:value-of select="@LENGTH"/>)</xsl:when>
  		<xsl:when test="@TYPENAME='DECIMAL'">(<xsl:value-of select="@LENGTH"/>,<xsl:value-of select="@SCALE"/>)</xsl:when>
	</xsl:choose>
	<xsl:if test="not(@NULLS='Y')"> NOT NULL</xsl:if>
...
ALTER TABLE <xsl:value-of select="$table"/>
	ADD CONSTRAINT "<xsl:value-of select="@CONSTNAME"/>" FOREIGN KEY 
		...
		REFERENCES "<xsl:value-of select="@REFTABSCHEMA"/>"."<xsl:value-of select="@REFTABNAME"/>" (<xsl:value-of select="@PK_COLNAMES"/>)
...
CREATE <xsl:if test="not(@UNIQUERULE='D')">UNIQUE</xsl:if> INDEX "<xsl:value-of select="@INDSCHEMA"/>"."<xsl:value-of select="$tableNamePrefix"/><xsl:value-of select="@INDNAME"/><xsl:value-of select="$tableNameSuffix"/>"
  ON <xsl:value-of select="$table"/>
...
<xsl:template name="grantAuthType">
	...
	<xsl:choose>
		<xsl:when test="$level = 'Y'">GRANT <xsl:value-of select="$auth"/>	on table <xsl:value-of select="$table"/> <xsl:call-template name="grantee"/> <xsl:value-of select="$delimiter"/><xsl:value-of select="$newLine"/></xsl:when>
		<xsl:when test="$level = 'G'">GRANT <xsl:value-of select="$auth"/>	on table <xsl:value-of select="$table"/> <xsl:call-template name="grantee"/> WITH GRANT OPTION <xsl:value-of select="$delimiter"/><xsl:value-of select="$newLine"/></xsl:when>
	</xsl:choose>
</xsl:template>
